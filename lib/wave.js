(function () {
  ...
  Wave.randomPosition = function (radius) {
    // var radius = 7;
    var randX = Math.floor(Math.random() * (500 - radius * 2)) + radius;
    var randY = Math.floor(Math.random() * (500 - radius * 2)) + radius;
    return [randX, randY];
  };

  Wave.randomAsteroidPos = function (width) {
    var randX = Math.floor(Math.random() * (500 - width)) + (width/2);
  ...
  Wave.prototype.movingObjects = function() {
    var result = [this.ship];
    return (result.concat(this.aliens).concat(this.bullets));
  };
