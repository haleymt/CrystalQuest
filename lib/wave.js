(function () {
  ...
  Wave.randomAsteroidPos = function (width) {
    ...
    } while (((randX < 30 || randX > 750)
              && (randY > CrystalQuest.Portal.RIGHT_POS
              && randY < CrystalQuest.Portal.RIGHT_POS + 60)))
    return [randX, randY];
  };
  ...
  Wave.prototype.draw = function(ctx) {
    ...
    // if (this.crystalPoints) {
    //   var num = Math.floor(Math.random() * (9000) + 1000);
    //   ctx.fillStyle = "#FFFFFF";
    //   ctx.font = "12pt Arial";
    //   ctx.fillText(num, this.ship.pos[0], this.ship.pos[1]);
    // }
  ...
  Wave.prototype.loseALife = function () {
    if ($('#lives').find('img').length === 0) {
      ...
      $('#life-count').html("")
      $('#lives').prepend('<img src="./life.png" width="25">');
      this.ship.relocate();
    } else {
  ...
  Wave.prototype.flashBomb = function () {
    if ($('#bombs').find('img').length > 0) {
      if ($('#bomb-count').text() === "") {
        ...
        $('#bomb-count').html("");
        $('#bombs').prepend('<img src="./bomb.png" height="15">');
        this.aliens.forEach( function (alien) {
          this.remove(alien);
        }.bind(this))
        this.aliens.forEach( function (alien) {
